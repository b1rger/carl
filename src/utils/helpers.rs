...
pub fn generate_dates_from_to(
    ...
    start_on_sunday: bool,
) -> Vec<Vec<chrono::NaiveDate>> {
    ...
    for month in months {
        ...
        let mut date = month
            .first_day_of_month()
            .first_day_of_week_before_first_day_of_month(start_on_sunday);
        while date
            <= month
                .last_day_of_month()
                .last_day_of_week_after_last_day_of_month(start_on_sunday)
        {
            month_v.push(date);
